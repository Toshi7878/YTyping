...
import z from "zod";
import { useSetVideoId, useVideoIdState } from "../../_lib/atoms/stateAtoms";
...
const TabInfoUpload = () => {
  ...
  const {
    ...
    useGeminiQueries().generateMapInfo({ videoId }, { enabled: !!(videoId && !isBackUp && hasUploadPermission) }),
  );
    ...
    <CardWithContent className={{ card: "py-3", cardContent: "flex flex-col gap-6" }}>
      <Form {...form}>
        <form action={formAction} className="flex w-full flex-col items-baseline gap-4">
          <FloatingLabelInputFormField name="video_id" label="動画ID" maxLength={11} />
