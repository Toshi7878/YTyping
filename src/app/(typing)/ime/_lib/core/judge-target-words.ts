...
import { updateImeTypeCountStats } from "../atoms/ref";
import {
  readBuiltMap,
  readStatus,
  readUtilityParams,
...
export const judgeTargetWords = (chatData: string) => {
  ...
  const { judgedWords } = readUtilityParams();
  ...
  for (const [i, judgedWord] of judgedWords.entries()) {
    ...
    updateImeTypeCountStats((prev) => prev + Math.round(wordTypeCount));
